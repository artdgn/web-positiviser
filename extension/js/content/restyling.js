...
class Restyler {
    ...
    static findAndRestyleAll_() {
      const scoreAttr = this.settings_.ranking ? 
          scoredTextsRankAtt : scoredTextsValueAtt;
    ...
    static updateElementColor_(element, score) {
      ...
      if (this.settings_.styling == 'color') {
        if (score >= threshold) {
          ...
          element.css(
              'background-color', `rgba(255, ${colorVal}, ${colorVal}, 0.4)`);
        } else {
          const colorVal = this.colorValue_(threshold - score, threshold);
          element.css(
              'background-color', `rgba(${colorVal}, 255, ${colorVal}, 0.4)`);
        }
    ...
    static updateElementVisibility_(element, score) {
      ...
      if ((this.settings_.styling == 'remove') && 
          (score >= (this.settings_.threshold))) {        
        element.hide(100);
